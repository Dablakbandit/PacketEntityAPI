...
public class Hitbox implements IHitbox{
	...
	public static IHitbox getByEntity(Entity entity){
		if(entity instanceof Slime){
			...
			return new Hitbox(new Vector(hb.min.getX() * size, hb.min.getY() * size, hb.min.getZ() * size), new Vector(hb.max.getX() * size, hb.max.getY() * size, hb.max.getZ() * size));
		}else if(entity instanceof AreaEffectCloud){
			Hitbox hb = boxes.get(EntityType.SLIME);
			float size = ((AreaEffectCloud)entity).getRadius();
			return new Hitbox(new Vector(hb.min.getX() * size, hb.min.getY(), hb.min.getZ() * size), new Vector(hb.max.getX() * size, hb.max.getY(), hb.max.getZ() * size));
		}else if(entity instanceof ArmorStand){
			ArmorStand as = (ArmorStand)entity;
			if(as.isMarker())
				return new Hitbox(0);
			if(as.isSmall())
				return new Hitbox(0.25, 0.9875);
			return new Hitbox(0.5, 1.975);
		}else if(entity instanceof Player){
			Player p = (Player)entity;
			if(p.isSneaking())
				return new Hitbox(0.6, 1.65);
			if(p.isGliding())
				return new Hitbox(0.6);
			if(p.isSleeping())
				return new Hitbox(0.2);
