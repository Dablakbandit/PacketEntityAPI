...
public class SightDistanceRegistry{
	...
	private static void loadSettings(String key, ConfigurationSection sec, ConfigurationSection defSec){
		if(!distances.containsKey(key))
			distances.put(key, new EnumMap<>(EntityType.class));
		...
		for(EntityType t : EntityType.values()){
			...
			}else if(ItemFrame.class.isAssignableFrom(clazz) || Painting.class.isAssignableFrom(clazz) || Item.class.isAssignableFrom(clazz) || ExperienceOrb.class.isAssignableFrom(clazz)){
	...
	public static Stream<Entity> getNearby(Player p, double error){
		...
		max *= error;
		return p.getNearbyEntities(max, 256, max).stream().filter(entity -> isNear(worldDistances.get(entity.getType()), p.getLocation(), entity.getLocation()));
	}
	...
	public static Stream<IEntityIdentifier> getNearby(Player p, double error, Collection<FakeEntity> fakes){
		...
		max *= error;
		Stream<IEntityIdentifier> real = p.getNearbyEntities(max, 256, max).stream().filter(entity -> isNear(worldDistances.get(entity.getType()), p.getLocation(), entity.getLocation())).map(EntityIdentifier::new);
		if(fakes == null){
			return real;
		}else{
			return Stream.concat(real, fakes.stream().filter(fakeEntity -> isNear(worldDistances.get(fakeEntity.getType()), p.getLocation(), fakeEntity.getLocation())).map(EntityIdentifier::new));
		}
	}
