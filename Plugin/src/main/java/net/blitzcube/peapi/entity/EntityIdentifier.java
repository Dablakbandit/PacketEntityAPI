...
public class EntityIdentifier implements IEntityIdentifier{
	...
	public void moreSpecific(){
		if(this.near == null)
			return;
		if(this.entity != null || this.fakeEntity != null)
			return;
		if(PacketEntityAPI.isFakeEntity(this.entityID)){
			...
			if(this.near.get() == null)
				return;
			Entity e = SightDistanceRegistry.getNearby(near.get(), 1.03).filter(en -> en.getEntityId() == entityID && (uuid == null || en.getUniqueId().equals(uuid))).findAny().orElse(null);
			if(e == null)
				return;
	...
	public boolean equals(Object o){
		...
		if((uuid != null && that.uuid != null) && !uuid.equals(that.uuid))
			return false;
		
		// If they both have specified nearby players and they are
		if((near != null && that.near != null) && near.get() != that.near.get())
			return false;
		
		// If they both have specified entity objects and they aren't equal
		if((entity != null && that.entity != null) && !Objects.equals(entity.get(), that.entity.get()))
			return false;
