...
public class PacketEventDispatcher{
	private static final Comparator<IListener>				LISTENER_COMPARATOR	= (o1, o2) -> IListener.ListenerPriority.getComparator().compare(o1.getPriority(), o2.getPriority());
	...
	public void add(IListener l){
		...
		if(l.shouldFireForFake()){
			if(sendingForFake == 0)
				engine.setSendForFake(true);
			sendingForFake++;
		}
		if(l.requiresCollidable()){
			if(requiresCollidable == 0)
				engine.setCollidable(true);
			requiresCollidable++;
		}
	}
	...
	public void dispatch(IEntityPacketEvent e, Boolean object){
		if(e == null || e.getPacket() == null)
			return;
		if(e.getPacketType().equals(IEntityPacketEvent.EntityPacketType.ENTITY_SPAWN) && e.getPacket() instanceof EntitySpawnPacket){
			...
			this.allListeners.stream().filter(iListener -> object == null || (object && objectListeners.contains(iListener)) || (!object && entityListeners.contains(iListener))).forEach(i -> i.onEvent(e));
		}
		e.context().execute();
	}
	...
	public void remove(IListener l){
		...
		if(l.shouldFireForFake()){
			sendingForFake--;
			if(sendingForFake == 0)
				engine.setSendForFake(false);
		}
		if(l.requiresCollidable()){
			requiresCollidable--;
			if(requiresCollidable == 0)
				engine.setCollidable(false);
		}
		boolean e = false, o = false;
		for(EntityType en : l.getTargets()){
			if(PacketEntityAPI.OBJECTS.containsKey(en)){
			...
			if(listenerLookup.get(en).size() == 0)
				listenerLookup.remove(en);
		}
		if(o){
			targetingObjects--;
