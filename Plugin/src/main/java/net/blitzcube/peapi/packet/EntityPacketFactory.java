...
public class EntityPacketFactory implements IEntityPacketFactory{
	@Override
	public IEntityAnimationPacket createAnimationPacket(IEntityIdentifier entity, IEntityAnimationPacket.AnimationType type){
		EntityAnimationPacket p = new EntityAnimationPacket(entity, EntityAnimationPacket.getEmptyPacket());
		p.setAnimation(type);
		return p;
	}
	...
	public IEntityClickPacket createClickPacket(IEntityIdentifier entity, IEntityClickPacket.ClickType type){
		EntityClickPacket p = new EntityClickPacket(entity, EntityClickPacket.getEmptyPacket());
		p.setClickType(type);
		return p;
	}
	...
	public IEntityDestroyPacket createDestroyPacket(IEntityIdentifier... entities){
		EntityDestroyPacket p = new EntityDestroyPacket(EntityDestroyPacket.getEmptyPacket());
		for(IEntityIdentifier i : entities)
			p.addToGroup(i);
		p.apply();
		return p;
	}
	...
	public IEntityEquipmentPacket[] createEquipmentPacket(IEntityIdentifier entity, EntityEquipment equipment){
		...
		for(int i = 0; i < 6; i++)
			packets[i] = new EntityEquipmentPacket(entity, EntityEquipmentPacket.getEmptyPacket());
	...
	public IEntityEquipmentPacket createEquipmentPacket(IEntityIdentifier entity, EquipmentSlot slot, ItemStack item){
		EntityEquipmentPacket p = new EntityEquipmentPacket(entity, EntityEquipmentPacket.getEmptyPacket());
		// Better way to do this?
		p.setItem(item);
		p.setSlot(slot);
		return p;
	...
	public IEntityMountPacket createMountPacket(IEntityIdentifier vehicle, IEntityIdentifier... passengers){
		EntityMountPacket p = new EntityMountPacket(vehicle, EntityMountPacket.getEmptyPacket());
		for(IEntityIdentifier passenger : passengers)
			p.addToGroup(passenger);
		p.apply();
		return p;
	}
	...
	public IEntitySpawnPacket createEntitySpawnPacket(IEntityIdentifier i){
		if(i.isFakeEntity()){
			...
			if(PacketEntityAPI.OBJECTS.containsKey(f.getType())){
				...
				EntitySpawnPacket p = new EntitySpawnPacket(i, EntitySpawnPacket.getEmptyPacket(f.getType()));
			...
			if(i.getEntity() == null)
				i.moreSpecific();
			Entity e;
			if(i.getEntity() == null || (e = i.getEntity().get()) == null)
				return null;
			if(PacketEntityAPI.OBJECTS.containsKey(e.getType())){
				throw new IllegalArgumentException("Tried to spawn an object with an entity packet!");
			}else{
				EntitySpawnPacket p = new EntitySpawnPacket(i, EntitySpawnPacket.getEmptyPacket(e.getType()));
	...
	public IEntityPacket[] createObjectSpawnPacket(IEntityIdentifier identifier){
		...
		if(identifier.isFakeEntity()){
			fake = true;
			if((f = identifier.getFakeEntity().get()) == null)
				return null;
			t = f.getType();
		}else{
			if(identifier.getEntity() == null)
				identifier.moreSpecific();
			if(identifier.getEntity() == null || (e = identifier.getEntity().get()) == null)
				return null;
			t = e.getType();
		}
		...
		if(t.equals(EntityType.PAINTING)){
			...
			p.setVelocity(fake ? (f.hasField("velocity") ? (Vector)f.getField("velocity") : new Vector(0, 0, 0)) : e.getVelocity());
	...
	public IEntityStatusPacket createStatusPacket(IEntityIdentifier identifier, byte status){
		EntityStatusPacket p = new EntityStatusPacket(identifier, EntityStatusPacket.getEmptyPacket());
		p.setStatus(status);
		return p;
	}
	...
	public IEntityPotionAddPacket createEffectAddPacket(IEntityIdentifier identifier, PotionEffect effect){
		EntityPotionAddPacket p = new EntityPotionAddPacket(identifier, EntityPotionAddPacket.getEmptyPacket());
		p.setEffect(effect);
		return p;
	}
	...
	public IEntityPotionRemovePacket createEffectRemovePacket(IEntityIdentifier identifier, PotionEffectType type){
		EntityPotionRemovePacket p = new EntityPotionRemovePacket(identifier, EntityPotionRemovePacket.getEmptyPacket());
		p.setEffectType(type);
		return p;
	}
	
	@Override
	public IEntityMovePacket createMovePacket(IEntityIdentifier identifier, Vector location, Vector direction, boolean onGround, IEntityMovePacket.MoveType type){
