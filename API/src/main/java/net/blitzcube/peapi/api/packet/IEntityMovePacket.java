...
public interface IEntityMovePacket extends IEntityPacket{
	...
	enum MoveType{
		REL_MOVE(NMSUtils.getInnerClass(NMSUtils.getNMSClass("PacketPlayOutEntity"), "PacketPlayOutRelEntityMove")), LOOK_AND_REL_MOVE(NMSUtils.getInnerClass(NMSUtils.getNMSClass("PacketPlayOutEntity"), "PacketPlayOutRelEntityMoveLook")),
		LOOK(NMSUtils.getInnerClass(NMSUtils.getNMSClass("PacketPlayOutEntity"), "PacketPlayOutEntityLook")), TELEPORT(NMSUtils.getNMSClass("PacketPlayOutEntityTeleport"));
		
		private final Class<?>			type;
		private final Constructor<?>	conPacket;
		
		MoveType(Class<?> type){
			this.type = type;
			this.conPacket = NMSUtils.getConstructor(type);
		}
		
		public WrappedPacket getEmptyPacket(){
			try{
				return new WrappedPacket(conPacket.newInstance());
			}catch(Exception e){
				e.printStackTrace();
			}
			return null;
		...
		public static MoveType getMoveType(Object packet){
			for(MoveType mt : MoveType.values()){
				if(mt.getPacketType().equals(packet.getClass())){ return mt; }
			}
			return null;
		}
